...
const text = computed(() => {
  ...
  if (tagRegExp.test(props.item.text)) {
    const result = props.item.text.match(tagRegExp)
    if (!result) {
      return props.item.text
    }
    const value = result[1].trim()
    const processText = props.item.text.replace(tagRegExp, '')
...
<template>
  ...
  >
    <div v-if="text" class="item" @click="onItemClick">
      <div class="indicator" />
      ...
      <VPLink
        ...
        :target="item.target"
      >
        <component :is="textTag" class="text" v-html="text" />
      </VPLink>
      <component :is="textTag" v-else class="text" v-html="text" />
